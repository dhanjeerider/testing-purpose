...
function appstorepro_fetch_generic_data( $url ) {
	...
	if ( is_wp_error( $response ) ) {
		return new WP_Error( 'http_error', __( 'Failed to fetch the page.', 'appstorepro' ) );
	}

	$code = wp_remote_retrieve_response_code( $response );
	if ( 200 !== (int) $code ) {
		return new WP_Error( 'http_error', sprintf( __( 'HTTP Error: %d', 'appstorepro' ), $code ) );
	}

	$html = wp_remote_retrieve_body( $response );
	if ( ! $html ) {
		return new WP_Error( 'no_content', __( 'The page returned no content.', 'appstorepro' ) );
	}
