 ...
 * 
 * Usage: [appstorepro_browse type="both" per_page="12"]
 * 
 * @param array $atts Shortcode attributes
 * @return string HTML
 */
function appstorepro_shortcode_browse( $atts ) {
	$atts = shortcode_atts( [
		'type'     => 'both', // 'both', 'app', 'game'
		'per_page' => 12,
		'orderby'  => 'date',
		'order'    => 'DESC',
	], $atts, 'appstorepro_browse' );

	ob_start();
	?>
	<div class="appstorepro-browse-wrapper">
		<!-- Hero Section -->
		<section class="browse-hero py-12 bg-gradient-to-b from-blue-50 to-white mb-10">
			<div class="max-w-6xl mx-auto px-4">
				...
				<!-- Search Form -->
				<form id="appstorepro-browse-form" class="bg-white rounded-2xl shadow-lg p-8">
					<!-- Search Bar -->
					<div class="mb-6">
						<div class="relative">
							<input 
		...
		<section class="max-w-6xl mx-auto px-4 pb-12">
			<div id="browse-results" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				<?php echo appstorepro_render_browse_results( $atts ); ?>
			</div>
	...
	<script>
		(function ($) {
			...
			$('#appstorepro-browse-form').on('submit', function (e) {
				e.preventDefault();
				appstorepro_load_browse_results();
			});
			...
			function appstorepro_load_browse_results() {
				var data = {
				...
				$.post(appstorepro.ajaxUrl, data, function (response) {
...
add_shortcode( 'appstorepro_browse', 'appstorepro_shortcode_browse' );

// ── Render browse results via AJAX ────────────────────────────────────────────
function appstorepro_render_browse_results( $args = [] ) {
	$defaults = [
...
function appstorepro_render_app_card() {
...
function appstorepro_ajax_browse_results() {
	check_ajax_referer( 'appstorepro_nonce', 'security' );
	appstorepro_render_browse_results();
	wp_die();
}
add_action( 'wp_ajax_appstorepro_browse_results', 'appstorepro_ajax_browse_results' );
add_action( 'wp_ajax_nopriv_appstorepro_browse_results', 'appstorepro_ajax_browse_results' );
 ...
 * 
 * Usage: [appstorepro_categories columns="3" show_count="yes"]
 */
function appstorepro_shortcode_categories( $atts ) {
	$atts = shortcode_atts( [
		'columns'   => 3,
		'show_count' => 'yes',
	], $atts, 'appstorepro_categories' );
	...
	?>
	<div class="appstorepro-categories-wrapper">
		<div class="categories-grid" style="display: grid; grid-template-columns: repeat(<?php echo (int) $atts['columns']; ?>, 1fr); gap: 24px; margin: 40px 0;">
			<?php foreach ( $categories as $category ) : ?>
				<?php
				$image_url = appstorepro_get_category_image( $category->term_id, 'medium' );
				...
				<a href="<?php echo esc_url( $link ); ?>" class="category-card">
					...
					<div class="category-info">
						...
						<?php if ( 'yes' === $atts['show_count'] ) : ?>
							<p><?php printf( _n( '%d app', '%d apps', $count, 'appstorepro' ), $count ); ?></p>
						<?php endif; ?>
					</div>
				</a>
			<?php endforeach; ?>
...
add_shortcode( 'appstorepro_categories', 'appstorepro_shortcode_categories' );
 ...
 * 
 * Usage: [appstorepro_home_hero]
 */
function appstorepro_shortcode_home_hero( $atts ) {
	ob_start();
	...
	?>
	<section class="appstorepro-home-hero py-12 bg-gradient-to-b from-blue-50 to-transparent">
		<div class="max-w-6xl mx-auto px-4">
			<!-- Hero Title -->
...
add_shortcode( 'appstorepro_home_hero', 'appstorepro_shortcode_home_hero' );
 ...
 * 
 * Usage: [appstorepro_home_collections]
 */
function appstorepro_shortcode_home_collections( $atts ) {
	ob_start();
		...
		<div class="max-w-6xl mx-auto px-4">
			...
			<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
				...
				foreach ( $categories as $category ) :
					$image_url = appstorepro_get_category_image( $category->term_id, 'large' );
					$link      = get_term_link( $category );
					?>
					<a href="<?php echo esc_url( $link ); ?>" class="collection-card relative rounded-2xl overflow-hidden aspect-square group cursor-pointer no-underline text-dark">
						...
						<div class="absolute bottom-0 left-0 right-0 p-4 sm:p-6 z-[2]">
							...
							<p class="text-white/80 text-sm mt-2 drop-shadow">
								<?php printf( _n( '%d app', '%d apps', $category->count, 'appstorepro' ), $category->count ); ?>
							</p>
						</div>
					</a>
				<?php endforeach; ?>
...
add_shortcode( 'appstorepro_home_collections', 'appstorepro_shortcode_home_collections' );
