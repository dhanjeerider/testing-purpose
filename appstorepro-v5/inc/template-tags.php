...
// aspv5_get_app_meta
function aspv5_get_app_meta( $post_id, $key ) {
	return get_post_meta( $post_id, $key, true );
}

// aspv5_get_app_rating_stars
function aspv5_get_app_rating_stars( $rating ) {
	$rating  = (float) $rating;
	$output  = '';
	$full    = floor( $rating );
...
// aspv5_format_downloads — compact format (1.2K / 3.4M)
function aspv5_format_downloads( $raw ) {
	$raw = trim( (string) $raw );
	if ( '' === $raw ) {
		return '';
	}
...
// aspv5_allowed_svg_kses (now also allows boxicons <i>)
function aspv5_allowed_svg_kses() {
	return [
...
// aspv5_get_category_icon_svg
function aspv5_get_category_icon_svg( $slug ) {
	$icons = [
		'game'          => 'bxs-joystick',
		'games'         => 'bxs-joystick',
...
// aspv5_get_color_themes
function aspv5_get_color_themes() {
	return [
